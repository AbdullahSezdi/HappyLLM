...
Soru: {question}"""
    ...
    def route_question(self, question: str) -> str:
        ...
        # Önce tam kalıp eşleşmesi kontrol et
        ...
        # Eğer kalıp eşleşmesi yoksa, genel QA'ya yönlendir
        return AgentType.QA
                            ...
                            if not country_data.empty:
                                ...
                                if len(countries) == 1 and countries[0] == 'Global Ortalama':
                                    ...
                                    for country in countries:
                                        country_yearly = country_data[country_data['country_name'] == country]
                                        if not country_yearly.empty:
                                            fig.add_trace(go.Scatter(
                                                x=country_yearly['year'],
                                                y=country_yearly[metric],
                                                mode='lines+markers',
                                                name=country,
                                                line=dict(width=3),
                                                marker=dict(size=8)
    ...
    async def get_answer(self, question: str) -> str:
        """Soruyu yanıtla - Optimize edilmiş versiyon"""
        agent_type = self.route_question(question)
        
        try:
            # Tüm sorular için genel LLM akışını kullan
            response = await self._get_llm_response(
                self.agents[agent_type],
                question=question,
                variables=list(self.df.columns) if agent_type == AgentType.CAUSAL else None
            )
